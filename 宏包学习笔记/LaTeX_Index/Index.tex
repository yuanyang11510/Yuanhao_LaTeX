...
\usepackage{imakeidx}
    ...
    % noautomatic参数用来将索引打印到正文部分之前（正常情况下只有在正文全部结束后才能打印索引），打印流程如下：
    % (1) 设置noautomatic参数后编译一次.tex文件，生成.idx文件
    % (2) 在当前文件夹中打开终端，使用makeindex命令生成.ind文件
    % (3) 再编译一次.tex文件，在正文部分之前的"\printindex"命令处会打印出索引
    %* 但是以上这种做法会导致许多索引的页码产生误差，因此不推荐使用
