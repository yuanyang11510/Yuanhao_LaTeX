...
\usepackage{titletoc}
    ...
    % \dottedcontents{section}[8em]{}{4em}{15pt}
        ...
        %* 这些参数包括：
            %* 目录中相应章节大纲【标签的右侧】/【标题文字的左侧】到文本左边线的距离；
            %* 章节整体的字体样式；
            %* 标签宽度（从【标签的右侧】/【标题文字的左侧】向左延伸的距离）；
            %* 圆点填充部分的等距填充的盒子宽度（对应"\titlerule*[width]{text}"命令中的[width]参数），{above-code}参数如果留空，会清空相应章节大纲的整体字体样式，使其还原为正文格式
