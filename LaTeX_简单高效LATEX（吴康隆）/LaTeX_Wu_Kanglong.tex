...
\usepackage{titlesec}
    ...
    \titleformat{\subsubsection}[display]{\itshape}{\bfseries 第\thesubsubsection 小节}{2em}{小节前$\leftarrow$\sffamily}[\rmfamily 小节后$\swarrow$]
            % 更加详细地设置章节的各项参数
            %* 关于参数设置，有一些细节值得指出：（1）{command}参数默认为设置有编号的大纲样式，可以在后面添加",numberless"，此时转为设置无编号的大纲样式；（2）{format}参数和{label}参数的情况和上一条命令一样，可以留空，但会导致相应的内容还原回正文字体；（3）设置{label}参数可以代替下文【通过重定义"\the+计数器名称"命令来修改章节标签】的方式，并且还有一个优势：这不会影响到此后所有级别更低的章节的标签，以上述命令为例，如果上述重定义了section的标签样式，这并不会影响到subsection的标签样式，但是如果像下文那样通过"\renewcommand{cmd}{def}"命令将section的标签重定义为了“第X章”的形式，这就不得不同时重定义subsection的标签形式，将其恢复为本来的形式（不过，只需要重定义比其低一级的subsection的标签形式即可，具体原因见下文）；（4）{sep}参数无法使用"\quad"等空格命令，但是可以使用"\baselineskip"命令
...
\renewcommand{\thesection}{第\arabic{section}章}
            %rfr 重定义"\the+计数器名称"命令不仅可以改变章节标签的数值形式，还可以改变章节标签的表述形式，比如上述命令中将"section"的表述形式从单纯的阿拉伯数字改为了"第+阿拉伯数字+章"的形式，可以参考：https://tex.stackexchange.com/questions/279744/change-the-name-of-sections
\renewcommand{\thesubsection}{\arabic{section}.\arabic{subsection}}
            %* "\thesubsection"命令本来的定义是"\thesection.\arabic{subsection}"，但是由于"\thesection"命令被重定义成了"第\arabic{section}章"的形式，因此为了让"\thesubsection"命令的打印形式保持不变，需要将其中的"\theseciton"替换成"\arbic{section}"
%// \renewcommand{\thesubsubsection}{\arabic{section}.\arabic{subsection}.\arabic{subsubsection}}
            %! "\thesubsubsection"命令不需要进行上述重定义，因为其本来的定义是"\thesubsection.\arabic{subsubsection}"，而上述重定义中的"\arabic{section}.\arabic{subsection}"部分其实就是重定义过后的"\thesubsection"命令
            %rfr 重定义计数器标签的命令可以总结成如下结构：\renewcommand{\the+计数器名称}{相关成分+\arabic{计数器名称}+相关成分}，关于"\the+<counter-name>"命令和"\arabic{counter-name}"命令这两类命令的不同，可以参见"3.7.1 计数器"一节     
...
% 可以使用multicol宏包提供的multicols环境，将页面分成任意多栏
    ...
    \clearpage 
            ...
            %? 这其实会引出一个问题：多栏环境中，系统如何决定多栏的高度？待研究
    ...
    \subsection{复杂距离}
        \subsubsection{水平和竖直距离}
            ...
            \begin{itemize}
                ...
                % 对应长度相等的"\enspace"命令
                %? 暂时还不知道"\enspace"命令和"\enskip"命令中的"en"指代的是什么
                \item a\ b，长度为一个空格距离
            \end{itemize}
...
\section[这是目录中显示的标题]{这是正文中显示的标题}
    ...
    \titlerule[.8pt]
            ...
            %? 是否可以通过某种方式在指定一级章节的下方填充直线或者填充内容，待研究
