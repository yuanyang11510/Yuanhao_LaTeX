...
\geometry{
    showframe,
        ...
        %! 注意，在设置这一参数时，页眉、页脚、侧页边根据是否被包括进页面区域的不同条件，会有各自不同的显示效果：（1）不管页眉是否被包括进页面区域，设置该参数都会显示整个页眉部分；（2）即使页脚没有被包括在页面区域内，设置该参数依然会显示页脚的上边线，一旦页脚被包括进页面区域，也仍然只显示其上边线；（3）即使侧页边没有被包括在页面区域内，设置该参数依然会显示侧页边的左边线，一旦侧页边被包括进页面区域内，则还会显示其右边线，这一点和页脚不同
    showcrop,
        %* 显示裁剪标志
% paper
...
% geometry将这一部分的参数称为"body size"，但随后又说明其实包含了整个"total body"（页面）的相关参数
    ...
    % total = {15cm,15cm},
        ...
        %! 注意，和paperwidth、paperheight、textwidth、textheight这些参数不同，这些参数都有对应的"\paperwidth"、"\paperheight"、"\textwidth"、"\textheight"命令，代表当前的纸张宽度、纸张高度、文本宽度、文本高度，(total)width和(total)height参数并不存在对应的"\(total)width"和"\(total)height"命令，因此在代码中如果使用这两个并不存在的命令，会导致汇编报错
    % text = {10cm,10cm},
        % 也可以设置名称为"body"
    ...
    includeall,
        ...
        %! 吴康隆P60称“默认的选项为includehead，表示总文本区包含页眉”，这句话其实是有问题的，geometry的宏包中在"includehead"参数后面明确提到，这一项默认设置为false，因此默认情况下其实页眉、页脚和侧页边都没有被包含在页面内
% totalbody&margin
...
% margin
    ...
    % marginratio = 3:2,
        % 也可以设置名称为"ration"，设置的值应该表现为小于100的正整数比，并且用":"符号连接
        %* 同时设置水平方向上的页面左边距和页面右边距的比值，以及竖直方向上的页面上边距和页面下边距的比值，前者的默认值在单边模式下为1:1，在双边模式下为2:3，后者的默认值为2:3，也可以单独设置hmarginration或者vmarginration
    % centering,
        %* 效果相当于marginration = 1:1，也可以单独设置hcentering（在单边模式下这是默认值）或者vcentering，要注意此处的"centering"并不是在浮动体环境中经常使用的"\centering"命令
    % twoside,
